...
type HomeProps = NativeStackScreenProps<RootStackParamList, 'MainTabs'>;

const { width, height } = Dimensions.get('window');

// ===== DADOS MOCKADOS DOS PETS =====
const PETS_DATA: Record<string, {
    "nome": string;
    "especie": string;
    "idade": string;
    "porte": string;
...
export function Home({ route, navigation }: HomeProps) {
    ...
    return (
        <View style={styles.root}>
            <Text style={styles.welcome}>Olá, {usuario}! Explore os pets disponíveis para adoção.</Text>
            <ScrollView style={styles.scroll}>
                {Object.entries(PETS_DATA).map(([id, pet]) => (
                    <TouchableOpacity key={id} style={styles.petCard} onPress={() => navigation.navigate('PetDetalhes', { petId: id, nomePet: pet.nome })}>
                        <Text style={styles.petEmoji}>{pet.emoji}</Text>
                        <View style={styles.petInfo}>
                            <Text style={styles.petNome}>{pet.nome}</Text>
                            <Text style={styles.petSub}>{pet.especie} · {pet.porte} · {pet.idade}</Text>
                            <Text style={styles.petCidade}>📍 {pet.cidade}</Text>
                        </View>
                    </TouchableOpacity>
                ))}
            </ScrollView>
